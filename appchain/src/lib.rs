...
pub mod pallet {
        ...
        fn offchain_worker(block_number: T::BlockNumber) {
            ...
            match Self::get_validator_id() {
                Some((public, key_data, validator_id)) => {
                    ...
                    log!(
                        debug,
                        "current mainchain_rpc_endpoint {:?}",
    ...
    impl<T: Config> ValidateUnsigned for Pallet<T> {
        ...
        fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
            ...
            if let Call::submit_observations {
                ref payload,
                ref signature,
            } = call
            {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn force_set_next_set_id(origin: OriginFor<T>, next_set_id: u32) -> DispatchResult {
            ...
            log!(
                info,
                "️️️force set next_set_id, next_set_id is : {:?} ",
                NextSetId::<T>::get()
            );
            Ok(())
        }
        ...
        pub fn tranfer_from_pallet_account(
            ...
            log!(
                debug,
                "Transfer from pallet, receiver: {:?}, amount: {:?} ",
                receiver,
                amount
            );
        ...
        pub fn force_mint_asset(
            ...
            Self::deposit_event(Event::ForceAssetMinted {
                asset_id,
                who,
                amount,
            });
    ...
    impl<T: Config> TokenIdAndAssetIdProvider<T::AssetId> for Pallet<T> {
        ...
        fn try_get_token_id(asset_id: T::AssetId) -> Result<Vec<u8>, Self::Err> {
            let token_id = <AssetIdByTokenId<T>>::iter()
                .find(|p| p.1 == asset_id)
                .map(|p| p.0);
            match token_id {
                Some(id) => Ok(id),
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn observing_mainchain(
            ...
            if obs.len() == 0 {
                log!(
                    debug,
                    "No validat_set updates, try to get appchain notifications."
                );
                // Make an external HTTP request to fetch the current price.
                // Note this call will block until response is received.
            ...
            if result[0].1.is_err() {
                log!(
                    warn,
                    "OCTOPUS-ALERT Failed to submit observations: {:?}",
                    result[0].1
                );
