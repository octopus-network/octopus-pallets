...
fn test_force_set_params() {
    ...
    new_tester().execute_with(|| {
        ...
        assert_ok!(OctopusAppchain::force_set_is_activated(
            Origin::root(),
            true
        ));
        ...
        assert_ok!(OctopusAppchain::force_set_next_notification_id(
            Origin::root(),
            10
        ));
        assert_noop!(
...
pub fn mock_payload_and_signature(
    ...
    ObservationsPayload<Public, BlockNumber, AccountId>,
    Signature,
) {
...
fn test_encode_args_works() {
    ...
    for (start, limit, expected) in test_get_notify_data {
        assert_eq!(
            expected,
            OctopusAppchain::encode_get_notification_args(start, limit)
        );
    }
}
