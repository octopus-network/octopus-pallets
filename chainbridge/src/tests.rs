...
fn whitelist_chain() {
    new_test_ext().execute_with(|| {
        ...
        assert_events(vec![Event::bridge(Event::ChainWhitelisted(0))]);
    })
}
...
fn set_get_threshold() {
    new_test_ext().execute_with(|| {
        ...
        assert_events(vec![
            Event::RelayerThresholdChanged(TEST_THRESHOLD),
            Event::RelayerThresholdChanged(5),
        ]);
    })
}
...
fn asset_transfer_success() {
    new_test_ext().execute_with(|| {
        ...
        assert_events(vec![
            Event::bridge(Event::ChainWhitelisted(dest_id.clone())),
            Event::bridge(Event::FungibleTransfer(
                dest_id.clone(),
                1,
                resource_id.clone(),
                amount.into(),
        ...
        assert_events(vec![Event::bridge(Event::NonFungibleTransfer(
            dest_id.clone(),
            2,
            resource_id.clone(),
            token_id,
        ...
        assert_events(vec![Event::bridge(Event::GenericTransfer(
            dest_id.clone(),
            3,
            resource_id,
            metadata,
...
fn asset_transfer_invalid_chain() {
    new_test_ext().execute_with(|| {
        ...
        assert_events(vec![Event::bridge(Event::ChainWhitelisted(
            chain_id.clone(),
        ))]);
...
fn add_remove_relayer() {
    new_test_ext().execute_with(|| {
        ...
        assert_events(vec![
            Event::Bridge(BridgeEvent::RelayerAdded(RELAYER_A)),
            Event::Bridge(BridgeEvent::RelayerAdded(RELAYER_B)),
            Event::Bridge(BridgeEvent::RelayerAdded(RELAYER_C)),
            Event::Bridge(BridgeEvent::RelayerRemoved(RELAYER_B)),
        ]);
    })
}
...
fn create_sucessful_proposal() {
    ...
    new_test_ext_initialized(src_id, r_id, b"System.remark".to_vec()).execute_with(|| {
        ...
        assert_events(vec![
            Event::Bridge(BridgeEvent::VoteFor(src_id, prop_id, RELAYER_A)),
            Event::Bridge(BridgeEvent::VoteAgainst(src_id, prop_id, RELAYER_B)),
            Event::Bridge(BridgeEvent::VoteFor(src_id, prop_id, RELAYER_C)),
            Event::Bridge(BridgeEvent::ProposalApproved(src_id, prop_id)),
            Event::Bridge(BridgeEvent::ProposalSucceeded(src_id, prop_id)),
        ]);
    })
}
...
fn create_unsucessful_proposal() {
    ...
    new_test_ext_initialized(src_id, r_id, b"System.remark".to_vec()).execute_with(|| {
        ...
        assert_events(vec![
            Event::Bridge(BridgeEvent::VoteFor(src_id, prop_id, RELAYER_A)),
            Event::Bridge(BridgeEvent::VoteAgainst(src_id, prop_id, RELAYER_B)),
            Event::Bridge(BridgeEvent::VoteAgainst(src_id, prop_id, RELAYER_C)),
            Event::Bridge(BridgeEvent::ProposalRejected(src_id, prop_id)),
        ]);
    })
}
...
fn execute_after_threshold_change() {
    ...
    new_test_ext_initialized(src_id, r_id, b"System.remark".to_vec()).execute_with(|| {
        ...
        assert_events(vec![
            Event::Bridge(BridgeEvent::VoteFor(src_id, prop_id, RELAYER_A)),
            Event::Bridge(BridgeEvent::RelayerThresholdChanged(1)),
            Event::Bridge(BridgeEvent::ProposalApproved(src_id, prop_id)),
            Event::Bridge(BridgeEvent::ProposalSucceeded(src_id, prop_id)),
        ]);
    })
...
fn proposal_expires() {
    ...
    new_test_ext_initialized(src_id, r_id, b"System.remark".to_vec()).execute_with(|| {
        ...
        assert_events(vec![Event::Bridge(BridgeEvent::VoteFor(
