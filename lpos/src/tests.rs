...
fn test_set_history_depth() {
	...
	new_tester().execute_with(|| {
		assert_ok!(OctopusLpos::set_history_depth(
			RuntimeOrigin::root(),
			EraIndex::zero(),
			u32::MAX
		));
...
fn test_force_set_era_payout() {
	let ferdie: AccountId = AccountKeyring::Ferdie.into();
	new_tester().execute_with(|| {
		assert_ok!(OctopusLpos::force_set_era_payout(RuntimeOrigin::root(), u128::MAX));

		assert_noop!(
			OctopusLpos::force_set_era_payout(RuntimeOrigin::signed(ferdie.clone()), u128::MAX),
			BadOrigin,
		);
	});
}
